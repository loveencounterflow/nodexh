(function() {
  'use strict';
  var CND, FS, PATH, _get_context, alert, badge, bold, cyan, debug, echo, exit_handler, fetch_mapped_location, get_context, get_error_callsites, gold, green, grey, help, info, load_source_map, log, red, reverse, rpr, show_error_with_source_context, steel, underline, urge, warn, whisper, white, write_to_stderr, yellow;
  ...
  show_error_with_source_context = async function(error, headline) {
    var arrowhead, arrowshaft, callsite, callsites, colnr, context_line, i, j, k, len, len1, len2, linenr, path, ref, relpath, width;
    ...
    for (i = 0, len = callsites.length; i < len; i++) {
      ...
      debug('^7776^', callsite.getFileName());
    }
//.........................................................................................................
    for (j = 0, len1 = callsites.length; j < len1; j++) {
      callsite = callsites[j];
  ...
  exit_handler = async function(error, origin) {
    var message, ref;
    debug('^exit_handler@33356756^', {origin});
    setTimeout((function() {
      return help('ok');
    }), 5000);
    ...
    debug('^33356756^', "show_error_with_source_context finished");
